...
subtitle: Spatially constrained methods has a hard requirement that spatial objects in the same cluster are also geographically linked.
...
Z-score normalization can be performed easily by using [*scale()*](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/scale) of Base R. The code chunk below will be used to perform the normalization of the clustering variables by using Z-score method. We will also use the [*describe()*](https://www.rdocumentation.org/packages/Hmisc/versions/4.4-0/topics/describe) from [**psych**](https://cran.r-project.org/web/packages/psych/) package instead of `summary()` for the report of the standard deviation.
...
Refer to my [RFM](https://jordan-isss624-geospatial.netlify.app/posts/me/rfm/) post for more explanation on these 3 methods. For this analysis we will use the gap statistic method.
...
The most interesting component of this list structure is the groups vector containing the labels of the cluster to which each observation belongs (as before, the label itself is arbitary). This is followed by a detaile

```{r}

```

d summary for each of the clusters in the edges.groups list. Sum of squares measures are given as ssto for the total and ssw to show the effect of each of the cuts on the overall criterion.
