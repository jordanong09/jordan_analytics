...
```{r}
h <- nigeria_localMI_dw[nigeria_localMI_dw$quadrant != 0,]
```
...
Once we have attained the Z score for each area, we will have to filter the area with z score less than the given critical value of [**3.886**](https://r-spatial.github.io/spdep/reference/localG.html). Since we are doing a two-tailed test, we will halve the critical value and check for **Z score < - CV/2** and **Z score > CV/2**. We will then visualise the variable using `tmap`.
