...
```{r}
head(table(hunan$County, card(wm_d62)),10)
```
...
From the result, we can see now the first area instead of **5** neighbours, it has **6** neighbours which include itself.
We will now retrieve the spatial window sum and combine it with our exisiting `Hunan` dataframe.
